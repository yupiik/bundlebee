...
@Dependent
public class HostIPC extends CheckValue {
    public HostIPC() {
        super(
    ...
    public String description() {
        return "Alert on pods/deployment-likes with sharing host's IPC namespace";
    }

    @Override
    public String remediation() {
        return "Ensure the host's IPC namespace is not shared.";
    }
    ...
    protected Stream<LintError> doValidate(final LintableDescriptor descriptor, final JsonValue value) {
        return value.getValueType() == JsonValue.ValueType.TRUE ?
                Stream.of(new LintError(WARNING, "hostIPC is true")) :
                Stream.empty();
    }
}
