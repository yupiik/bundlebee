...
public class MinfFunc implements HelmFunction {
    @Override
    public String name() {
        return "minf";
    }
    ...
    public Object execute(final Object... args) {
        if (args == null || args.length < 1) {
            throw new IllegalArgumentException("minf requires at least one argument");
        }
        var result = toDouble(args[0]);
        for (var i = 1; i < args.length; i++) {
            result = Math.min(result, toDouble(args[i]));
        }
        return result;
    }
