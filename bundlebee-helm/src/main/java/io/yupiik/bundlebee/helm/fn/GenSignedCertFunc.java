...
public class GenSignedCertFunc implements HelmFunction {
    @Override
    public String name() {
        return "genSignedCert";
    }
    ...
    public Object execute(final Object... args) {
        if (args.length < 1 || args[0] == null) {
            throw new IllegalArgumentException("genSignedCert requires at least cn argument");
        }
        final var cn = args[0].toString();
        final var ips = args.length >= 2 && args[1] != null ? toStringList(args[1]) : List.<String>of();
        final var dns = args.length >= 3 && args[2] != null ? toStringList(args[2]) : List.<String>of();
        final var days = args.length >= 4 && args[3] != null ? toInt(args[3]) : 365;
        final var caPem = args.length >= 5 && args[4] != null ? args[4].toString() : null;
        ...
        try {
            ...
            // Generate a new self-signed cert (CA-signed is complex without BouncyCastle)
            // This produces a self-signed cert as a placeholder
            ...
            throw new IllegalStateException("Failed to generate signed certificate", e);
        }
    }
